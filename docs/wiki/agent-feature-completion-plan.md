...
- **隐私边界（已选定 A）**：评测结果**只**在开发者本机或 CI 中生成；**生产应用不上报任何指标**——
  无遥测、无分析、无崩溃上报，与 `docs/privacy.md` 一致。选项 (B)（将来允许聚合指标）已否决：
  除非先按 `privacy.md` 的决策节改写隐私文档并连同同意 UI 与删除路径一起交付，否则方案页不得
  出现“上报”字样。场景只使用合成数据，不包含真实用户学习内容。
