...
7. **已知限制**：未合并；仍是 deterministic、同步、JSON-only runner，不含真实模型评测、provider 或
   工具安全套件。**遥测表述冲突**：方案页 AG10 写「生产只上报聚合指标」，而
   `docs/privacy.md` 第 8 行写死 "No telemetry. No analytics."——在本页收敛前，以 privacy.md 为准，
   即**当前没有任何上报**；未来若要上线聚合指标，必须重新定义默认关闭、同意方式、去向、保留期、
   删除方式与绝不上传字段。
8. **依赖**：横切全阶段。
