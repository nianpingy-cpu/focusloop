...
/** Runs AG2 fixtures against production routing, plan construction and success evaluation. */
export function runAg2Adapter(input: JsonValue): JsonValue {
  ...
  const reason: StuckReason | null = isStuckReason(reasonValue) ? reasonValue : null;
  const stateValue = input['state'];
  const state: LearningState = isLearningState(stateValue) ? stateValue : 'FOCUSED';
  ...
  // Keep this adapter on the same production route as the app: a stated reason is a
  // HELP_REQUESTED event consumed by decideIntervention, rather than a direct lookup in the
  // rescue table. This is important for state precedence (for example OVERLOADED) and for the
  // policy's answersRequestId bookkeeping.
  const requestEvent = {
    id: requestId,
